...
SCENARIOS = {
    "B1": (MRIOClubCBAM, {}, "#1f77b4", "Single club"),
    "B2": (MRIOSectoralClub, {}, "#2ca02c", "Sectoral club"),
    "B3": (
        MRIOMultiClub,
        {"club_cores": (EU_IDX, B3_SECOND_CORE)},
        "#d62728",
        "Competing clubs",
    ),
}
...
def _plot_results(results, timestamp, bau_temp=None):
    ...
    gs = gridspec.GridSpec(
        4, 3, figure=fig, hspace=0.60, wspace=0.30, height_ratios=[1, 1, 1, 0.7]
    )
    ...
    for ax, (key, title, hint) in zip(panel_axes, _KPI_PANELS):
        ...
        if key == "temp_rise":
            ...
            if bau_temp is not None:
                ...
                ax.annotate(
                    f"no-policy BAU  {bau_temp:.2f}°C",
                    (len(labels) - 0.5, bau_temp),
                    ha="right",
                    va="bottom",
                    fontsize=8,
                    color="0.35",
            ...
            cut_txt = (
                f"CBAM cuts only {cbam_cut:.2f}°C off BAU\n"
                if cbam_cut is not None
                else ""
            )
            ax.annotate(
    ...
    tbl = ax_tbl.table(
        cellText=table_data, colLabels=col_heads, loc="center", cellLoc="center"
    )
