...
Usage (from rice_jax/, rice-jax conda env):
    python validation/cbam_litmus_mechanism.py [--timesteps 2000000]
    python validation/cbam_litmus_mechanism.py --replot <pickle.pkl>
"""
...
def _mean_mu_non_eu(mitigation, last_t=EVAL_LAST_T):
    ...
    return _metrics.mean_mitigation_rate(
        mitigation, region_idxs=CBAM_PLOT_REGIONS, last_t=last_t,
    )
...
def run_m1(key):
    ...
    env_diff = _build_env("differential", False, True, FIXED_SAVINGS,
    ...
    raw_ctrl = _build_env("flat", False, True, FIXED_SAVINGS,
                          cbam_tariff_rate=0.0, for_training=False)
    raw_diff = _build_env("differential", False, True, FIXED_SAVINGS,
    ...
    return passed, dict(
        ...
        _agent_ctrl=agent_ctrl.state, _agent_diff=agent_diff.state,
    )
...
def run_m2(key):
    ...
    num_iters = TOTAL_TIMESTEPS // NUM_ENVS // NUM_STEPS
    env = _build_env("differential", True, False, FIXED_SAVINGS, delta_max=0.0,
                     cbam_lambda_init=CBAM_LAMBDA_INIT)
    agent, csv_path = _train("m2_costless_mu", env, key, num_iters)

    eval_key = jax.random.fold_in(key, 20)
    raw_env  = _build_env("differential", True, False, FIXED_SAVINGS, delta_max=0.0,
                          cbam_lambda_init=CBAM_LAMBDA_INIT, for_training=False)
...
def run_m3(key, m2_mu=None):
    ...
    num_iters = TOTAL_TIMESTEPS // NUM_ENVS // NUM_STEPS
    env = _build_env("differential", False, False, FIXED_SAVINGS, delta_max=0.0,
                     cbam_lambda_init=CBAM_LAMBDA_INIT)
    agent, csv_path = _train("m3_costly_mu", env, key, num_iters)

    eval_key = jax.random.fold_in(key, 30)
    raw_env  = _build_env("differential", False, False, FIXED_SAVINGS, delta_max=0.0,
                          cbam_lambda_init=CBAM_LAMBDA_INIT, for_training=False)
    ev = _eval_episode(eval_key, raw_env, agent)
    ...
    if m2_mu is not None:
        ...
        print(f"         Secondary: μ_M3={mean_mu:.3f} vs μ_M2={m2_mu:.3f}  "
...
def run_m4(key, m3_mu):
    ...
    num_iters = TOTAL_TIMESTEPS // NUM_ENVS // NUM_STEPS
    env = _build_env("differential", False, False, FIXED_SAVINGS,
                     cbam_lambda_init=CBAM_LAMBDA_INIT)
    agent, csv_path = _train("m4_both", env, key, num_iters)

    eval_key = jax.random.fold_in(key, 40)
    raw_env  = _build_env("differential", False, False, FIXED_SAVINGS,
                          cbam_lambda_init=CBAM_LAMBDA_INIT, for_training=False)
    ev = _eval_episode(eval_key, raw_env, agent)
...
def run_n1(key):
    ...
    cost_low_total  = float(sum(
        np.array(cost_dict_low[r])[step_idx] for r in NON_EU))
    cost_high_total = float(sum(
        np.array(cost_dict_high[r])[step_idx] for r in NON_EU))
    ...
    print(f"         Higher μ {'reduces' if passed else 'does NOT reduce'} CBAM cost mechanically")

    return passed, dict(
        cost_low=cost_low_total, cost_high=cost_high_total,
    )
...
def plot_results(results, timestamp):
    ...
    if "m2" in r:
        m2 = r["m2"]["data"]
        _plot_convergence(ax_m2_conv, [m2["csv_path"]], ["M2 costless abatement"],
                          ["#27ae60"], "M2: Training convergence")
    if "m3" in r:
        m3 = r["m3"]["data"]
        _plot_convergence(ax_m3_conv, [m3["csv_path"]], ["M3 costly abatement"],
                          ["#f39c12"], "M3: Training convergence")
    ...
    if "m4" in r:
        m4 = r["m4"]["data"]
        _plot_convergence(ax_m4_conv, [m4["csv_path"]], ["M4 both channels"],
                          ["#8e44ad"], "M4: Training convergence")
    ...
    if "n1" in r and not r["n1"]["data"].get("skipped", False):
        ...
        ax_n1.set_xticklabels(["μ_low (no mitigation)", "μ_high (saturated)"], fontsize=8)
        ax_n1.set_ylabel("CBAM cost (step 1)", fontsize=8)
        ax_n1.set_title("N1: Mechanical Tariff-Relief Null\n"
                        "(higher μ → lower τ_eff → lower CBAM cost, no RL)", fontsize=9)
        ax_n1.grid(True, axis="y", lw=0.4, alpha=0.4)
    else:
        ax_n1.axis("off")
        ax_n1.text(0.5, 0.5, "N1 skipped or not run", ha="center", va="center",
                   transform=ax_n1.transAxes, fontsize=10, color="grey")
    ...
    if "m2" in r:
        tests.append(("M2 Costless μ exists", r["m2"]["passed"],
                       f"mean μ={r['m2']['data']['mean_mu']:.3f} (>0.15)"))
    if "m3" in r:
        m3d = r["m3"]["data"]
        m2_str = f"{r['m2']['data']['mean_mu']:.3f}" if "m2" in r else "?"
        tests.append(("M3 Costly μ survives", r["m3"]["passed"],
                       f"mean μ={m3d['mean_mu']:.3f} (>0.01); "
                       f"cf. M2={m2_str} (descriptive)"))
    if "m4" in r:
        m3_ref = r["m3"]["data"]["mean_mu"] if "m3" in r else "?"
        tests.append(("M4 Diversion crowds out μ", r["m4"]["passed"],
                       f"mean μ={r['m4']['data']['mean_mu']:.3f} "
                       f"(< M3={m3_ref})"))
    if "n1" in r:
        n1d = r["n1"]["data"]
        if n1d.get("skipped"):
            ...
            tests.append(("N1 Tariff-relief null", r["n1"]["passed"],
...
def main():
    ...
    parser.add_argument("--tests",     type=str, default="m1,m2,m3,m4,n1",
    ...
    sav_group.add_argument("--fixed-savings", dest="fixed_savings",
                           action="store_true", default=True,
                           help="Fix savings_rate to 0.2 (default)")
    sav_group.add_argument("--free-savings",  dest="fixed_savings",
                           action="store_false",
    ...
    save_run_config({
        "script": "validation/cbam_litmus_mechanism.py",
