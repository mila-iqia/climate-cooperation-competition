...
Usage (from rice_jax/, rice-jax conda env):
    python validation/cbam_litmus_conditioning.py [--timesteps 2000000]
    python validation/cbam_litmus_conditioning.py --replot <pickle.pkl>
"""
...
def _per_region_eu_share_by_sector(trade_flows, last_t=5):
...
def run_c1(key):
    ...
    env = _build_env("differential", False, True, FIXED_SAVINGS,
    ...
    raw_on  = _build_eval_env("differential", False, True, FIXED_SAVINGS,
                              cbam_lambda_init=CBAM_LAMBDA_INIT, force_rate=1.0)
    raw_off = _build_eval_env("differential", False, True, FIXED_SAVINGS,
    ...
    return passed, dict(
        share_on=share_on, share_off=share_off,
        cond_gap=cond_gap, cond_gap_pp=(share_off - share_on) * 100,
...
def run_c2(key):
    ...
    # ── C2a: costless ──
    env_a = _build_env("differential", True, False, FIXED_SAVINGS, delta_max=0.0,
                       cbam_lambda_init=CBAM_LAMBDA_INIT, randomize=True)
    agent_a, csv_a = _train("c2a_costless", env_a, key_a, num_iters)

    eval_key = jax.random.fold_in(key, 20)
    raw_a_on  = _build_eval_env("differential", True, False, FIXED_SAVINGS, delta_max=0.0,
                                cbam_lambda_init=CBAM_LAMBDA_INIT, force_rate=1.0)
    raw_a_off = _build_eval_env("differential", True, False, FIXED_SAVINGS, delta_max=0.0,
                                cbam_lambda_init=CBAM_LAMBDA_INIT, force_rate=0.0)
    ...
    # ── C2b: costly ──
    env_b = _build_env("differential", False, False, FIXED_SAVINGS, delta_max=0.0,
                       cbam_lambda_init=CBAM_LAMBDA_INIT, randomize=True)
    agent_b, csv_b = _train("c2b_costly", env_b, key_b, num_iters)

    raw_b_on  = _build_eval_env("differential", False, False, FIXED_SAVINGS, delta_max=0.0,
                                cbam_lambda_init=CBAM_LAMBDA_INIT, force_rate=1.0)
    raw_b_off = _build_eval_env("differential", False, False, FIXED_SAVINGS, delta_max=0.0,
                                cbam_lambda_init=CBAM_LAMBDA_INIT, force_rate=0.0)
    ...
    print(f"  C2b ({grade_b}): μ(cbam=1)={mu_b_on:.3f}  μ(cbam=0)={mu_b_off:.3f}  "
          f"gap={gap_b:.3f}")

    # Overall C2 grade: take the stronger of the two
    grades_order = {"none/mixed": 0, "weak": 1, "strong": 2}
    overall_grade = grade_a if grades_order[grade_a] >= grades_order[grade_b] else grade_b

    return overall_grade, dict(
        # C2a
        mu_a_on=mu_a_on, mu_a_off=mu_a_off, gap_a=gap_a, grade_a=grade_a,
        pr_mu_a_on=_per_region_mu(ev_a_on["mitigation"]),
        pr_mu_a_off=_per_region_mu(ev_a_off["mitigation"]),
        csv_a=csv_a,
        _agent_a=agent_a.state,
        # C2b
        mu_b_on=mu_b_on, mu_b_off=mu_b_off, gap_b=gap_b, grade_b=grade_b,
        pr_mu_b_on=_per_region_mu(ev_b_on["mitigation"]),
        pr_mu_b_off=_per_region_mu(ev_b_off["mitigation"]),
        csv_b=csv_b,
        _agent_b=agent_b.state,
        # Overall
...
def run_c3(key, c2b_mu_on):
    ...
    raw_on  = _build_eval_env("differential", False, False, FIXED_SAVINGS,
                              cbam_lambda_init=CBAM_LAMBDA_INIT, force_rate=1.0)
    raw_off = _build_eval_env("differential", False, False, FIXED_SAVINGS,
                              cbam_lambda_init=CBAM_LAMBDA_INIT, force_rate=0.0)
    ...
    print(f"         Conditioned crowd-out: μ drops {c2b_mu_on - mu_on:.3f} "
          f"({'present' if passed else 'absent'})")

    return passed, dict(
        mu_on=mu_on, mu_off=mu_off,
...
def _bar_per_region(ax, series_dict, title, ylabel, colors=None):
    ...
    for i, (name, series) in enumerate(series_dict.items()):
        ...
        ax.bar(x + i * w - (n - 1) * w / 2, vals, w * 0.9,
               label=name, color=_colors[i], alpha=0.85)
    ax.set_xticks(x)
...
def plot_results(results, timestamp):
    ...
    if "c1" in r:
        c1 = r["c1"]["data"]
        _plot_convergence(ax_c1_conv, [c1["csv_path"]],
                          ["C1 randomised diff"], ["#e74c3c"],
                          "C1: Training convergence")
        _bar_per_region(ax_c1_bar,
                        {"cbam=0 (off)": c1["pr_off"],
                         "cbam=1 (on)":  c1["pr_on"]},
    ...
    if "c2" in r:
        c2 = r["c2"]["data"]
        _plot_convergence(ax_c2a_conv, [c2["csv_a"]],
                          ["C2a costless"], ["#27ae60"],
                          "C2a: Convergence (costless)")
        _plot_convergence(ax_c2b_conv, [c2["csv_b"]],
                          ["C2b costly"], ["#f39c12"],
                          "C2b: Convergence (costly)")
        _bar_per_region(ax_c2_bar,
    ...
    if "c3" in r:
        c3 = r["c3"]["data"]
        _plot_convergence(ax_c3_conv, [c3["csv_path"]],
                          ["C3 both channels"], ["#8e44ad"],
                          "C3: Convergence")

    if "c2" in r and "c3" in r:
        c2 = r["c2"]["data"]
        c3 = r["c3"]["data"]
        _bar_per_region(ax_c3_mu,
    ...
    if "c3" in r:
        _bar_per_region(ax_c3_share,
                        {"C3 EU dirty share (cbam=1)": r["c3"]["data"]["pr_share_on"]},
                        "C3: EU dirty share\n(cbam=1, both open)",
                        "EU dirty share", colors=["#8e44ad"])
...
def main():
    ...
    parser.add_argument("--tests",     type=str, default="c1,c2,c3",
    ...
    save_run_config({
        "script": "validation/cbam_litmus_conditioning.py",
