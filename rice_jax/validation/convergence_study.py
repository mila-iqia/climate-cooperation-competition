...
Usage:
    ...
    python validation/convergence_study.py

    # With differential CBAM
    python validation/convergence_study.py --cbam-mode differential

    # Three canonical seeds, with a shorter smoke-test budget
    python validation/convergence_study.py --timesteps 100000 --seeds 0 1 2

    # Regenerate plot from existing CSV
    python validation/convergence_study.py --plot-only training_logs/convergence_no_cbam_XYZ.csv
"""
...
# Per agent (alphabetical key order when JAX flattens the action dict):
#   export_reallocation [2*NR] = sector0[NR dests] + sector1[NR dests]
#   mitigation_rate     [1]
#   savings_rate        [1]
# Total: (2*NR+2) dims × NR agents.
_N_SECTORS = 2             # emissions-simple: dirty, clean
_SECTOR_NAMES = ["dirty(CBAM)", "clean"]

def _action_layout():
    dims_per_agent = _N_SECTORS * NUM_REGIONS + 2
    mitig_offset = _N_SECTORS * NUM_REGIONS
    savings_offset = mitig_offset + 1
    return dims_per_agent, mitig_offset, savings_offset
...
def build_env(cbam_rate: float = 0.0, log_info_fn=None, wrap: bool = True,
    ...
    from validation.canonical_config import CANONICAL_YAML_DIR, make_canonical_env
    ...
    if log_info_fn is not None:
        env = replace(env, log_info_fn=log_info_fn)
    return env
...
def build_ppo(total_timesteps: int, csv_path: str):
    from training_monitor import RCPOMonitoredPPO, make_combined_log_fn, make_csv_log_fn, make_print_log_fn
    from validation.canonical_config import CANONICAL_TRAIN_KWARGS
    ...
    ppo_kwargs.update(total_timesteps=total_timesteps, log_function=log_fn)
...
def _trade_flows_log_fn(state: dict, actions: dict) -> dict:
...
def eval_eu_shares(ppo, cbam_rate: float, key, *, flat_tariff: bool = False,
                   cbam_mode: str | None = None,
    ...
    for ep in range(NUM_EVAL_EPISODES):
        ...
        logs = run_single_episode(ep_key, eval_env, ppo)
...
def make_convergence_plot(csv_path: str, plot_path: str, cbam_rate: float = 0.0,
    ...
    NS = _N_SECTORS    # 2
    B = _N_SECTORS * N + 2
    _MITIG_OFF = _N_SECTORS * N
    _SAVINGS_OFF = _MITIG_OFF + 1
    max_dim = total_dims - 1

    # ── Per-agent export means (avg over destinations within each sector) ─────
    # agent a, sector s → cols [a*B + s*N .. a*B + (s+1)*N - 1]
    def _agent_sector_mean(a, s, arr):
        cols = [a * B + s * N + d for d in range(N) if a * B + s * N + d <= max_dim]
        return arr[:, cols].mean(axis=1) if cols else np.full(len(iters), np.nan)

    # ── Cross-agent aggregates for mitigation/savings ─────────────────────────
    def _agent_offsets(within_block):
        return [a * B + off for a in range(N) for off in within_block
                if a * B + off <= max_dim]

    def _mean_agg(indices, arr):
        return arr[:, indices].mean(axis=1) if indices else np.full(len(iters), np.nan)

    mitig_mean   = _mean_agg(_agent_offsets([_MITIG_OFF]),   act_means)
    savings_mean = _mean_agg(_agent_offsets([_SAVINGS_OFF]), act_means)
    mitig_std    = _mean_agg(_agent_offsets([_MITIG_OFF]),   act_stds)
    savings_std  = _mean_agg(_agent_offsets([_SAVINGS_OFF]), act_stds)
    ...
    ax.set_title("Episode return — full training  "
                 "(step reward is normalised by PPO and always ≈0)")
...
def main():
    ...
    for seed in seeds:
        ...
        ppo = build_ppo(args.timesteps, csv_path)
        ppo = ppo.train(jax.random.PRNGKey(seed), env)
        csv_paths.append(csv_path)
        trained.append((ppo, seed))
    ...
    seed_shares = [
        eval_eu_shares(
            ppo, args.cbam_rate, jax.random.PRNGKey(seed),
            yaml_dir=args.yaml_dir, flat_tariff=args.flat_tariff,
            cbam_mode=cbam_mode,
        )
        for ppo, seed in trained
    ]
