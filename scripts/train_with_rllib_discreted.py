...
from rice_discreted import Rice
...
def trainer(
    ...
    # -----------------------------------------------------------------------------
    config_path = os.path.join(PUBLIC_REPO_DIR, "scripts", "rice_rllib_discreted.yaml")
    ...
    with open(os.path.join(save_dir, "rice_rllib_discreted.yaml"), "w") as yaml_file:
        yaml.dump(run_config, yaml_file)
    # Copy source files to the saving directory
    for file in ["rice_discreted.py", "rice_helpers.py"]:
        shutil.copyfile(
...
if __name__ == "__main__":
    ...
    for file in ["rice_rllib_discreted.yaml"]:
        shutil.copyfile(
