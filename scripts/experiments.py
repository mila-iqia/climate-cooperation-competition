...
def run_carbon_leakage(trainer_obj=None,
    ...
    second_club = env_object.env.club_members
    ...
    for timestep in range(env.episode_length):
        ...
        for region_id in range(env.num_agents):
            ...
            if not env.control:
                if region_id in env.club_members:
                    if region_id in club_test[condition].keys():
                        club_test[condition][region_id].append(actions[region_id][1])
                    else:
                        club_test[condition][region_id] = [actions[region_id][1]]
