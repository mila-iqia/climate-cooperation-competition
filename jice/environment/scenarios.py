...
class BasicClub(Rice):
    ...
    def club_members(self) -> np.ndarray:
        return np.array(
            [region_id for region_id in range(self.num_regions) if region_id in self.club_members_]
        )

    @property
    def non_club_members(self) -> list:
        return np.array(
            [region_id for region_id in range(self.num_regions) if region_id not in self.club_members]
        )
    ...
    def generate_action_masks(self, state: EnvState) -> chex.Array:
        ...
        non_club_member_tariff_action_indices = first_tariff_action_index + self.non_club_members
        
        if self.promote_free_trade_among_club_members:
            club_member_tariff_action_indices = first_tariff_action_index + self.club_members
        ...
        min_tariff_amount_per_region = (self.club_mitigation_rate - state.mitigation_rates_all_regions).astype(int)
        min_tariff_amount_per_region = min_tariff_amount_per_region.clip(min=0)
        mask_per_region = jnp.arange(self.num_discrete_action_levels) >= min_tariff_amount_per_region[:, None]
        action_mask = action_mask.at[
